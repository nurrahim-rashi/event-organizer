...
export default function EventStatistics({ eventId }: EventStatisticsProps) {
  ...
  <div className="p-6 bg-purple-950/20 rounded-2xl border border-purple-900/30 text-white space-y-6">
    ...
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div>
        <h2 className="text-xl font-bold text-white">Statistics Visualization</h2>
        <p className="text-xs text-purple-300/70">Displaying graphical reports by day, month, and year</p>
      </div>
    ...
    {loading ? (
      <div className="h-64 flex items-center justify-center">
        <p className="text-sm text-purple-300 animate-pulse">Loading graphical data...</p>
      </div>
    ) : chartData.length === 0 ? (
      <div className="h-64 flex items-center justify-center bg-purple-950/10 rounded-xl border border-dashed border-purple-900/30">
        <p className="text-sm text-gray-400 italic">No transaction data found for this period.</p>
      </div>
    ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-purple-950/40 p-4 rounded-xl border border-purple-900/20">
          <h3 className="text-sm font-medium text-purple-300 mb-4 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-yellow-400"></span> Total Revenue
          </h3>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#3b0764" vertical={false} />
                <XAxis dataKey="label" stroke="#a78bfa" fontSize={11} tickLine={false} />
                <YAxis 
                  stroke="#a78bfa" 
                  fontSize={11} 
                  tickLine={false} 
                  tickFormatter={(v) => `Rp ${v >= 1000000 ? (v / 1000000) + 'M' : v.toLocaleString('id-ID')}`} 
                />
                <Tooltip 
                  contentStyle={{ backgroundColor: '#1e1b4b', borderColor: '#581c87', borderRadius: '12px' }} 
                  formatter={(value: any) => [formatRupiah(Number(value)), "Revenue"]}
                />
                <Line type="monotone" dataKey="revenue" stroke="#facc15" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-purple-950/40 p-4 rounded-xl border border-purple-900/20">
          <h3 className="text-sm font-medium text-purple-300 mb-4 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-purple-400"></span> Tickets Sold
          </h3>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#3b0764" vertical={false} />
                <XAxis dataKey="label" stroke="#a78bfa" fontSize={11} tickLine={false} />
                <YAxis stroke="#a78bfa" fontSize={11} tickLine={false} allowDecimals={false} />
                <Tooltip 
                  contentStyle={{ backgroundColor: '#1e1b4b', borderColor: '#581c87', borderRadius: '12px' }}
                  formatter={(value: any) => [`${value} Tickets`, "Sold"]}
                />
                <Bar dataKey="ticketsSold" fill="#c084fc" radius={[6, 6, 0, 0]} maxBarSize={40} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
